...
bool XmlParser::isCorrectModule(const QString &typem, const QString &typeb,
                                quint16 m_typem, quint16 m_typeb) {
...
ctti::unnamed_type_id_t XmlParser::parseType(QDomElement domElement) {
  ...
  case 0:
    ...
    if (name.contains("M_SP", Qt::CaseInsensitive))
      return ctti::unnamed_type_id<DataTypes::SinglePointWithTimeStruct>()
          .hash();
...
config::itemVariant XmlParser::parseWidget(QDomElement domElement) {
  ...
  auto widgetGroup =
      static_cast<delegate::WidgetGroup>(childElement.text().toInt());
  ...
  case ctti::unnamed_type_id<QComboBox>().hash(): {
    ...
    //    = childElement.text().contains("data") ? delegate::QComboBox::data :
    //    delegate::QComboBox::index;
    return std::move(widget);
  }
...
void XmlParser::mergeWidget(const QDomElement &domElement,
                            config::widgetMap *const s2widgetMap,
                            BciNumber id) {
...
DataTypes::RecordPair
XmlParser::parseRecordConfig(QDomElement domElement,
                             config::widgetMap *const s2widgetMap) {
  using namespace DataTypes;
  ...
  Q_ASSERT(false && "Wrong visible value: " &&
           defaultValue.toStdString().c_str());
  return RecordPair{DataTypes::DataRecV(id, defaultValue), true};
}

check::detail::Record
XmlParserHelper::parseRecordCheck(QDomElement domElement) {
  ...
  rec.start_addr =
      domElement.firstChildElement("start-addr").text().toUShort(&ok);
  assert(ok);
...
config::Item XmlParser::parseItem(QDomElement domElement,
                                  ctti::unnamed_type_id_t parentType) {
  using namespace config;
  auto name = domElement.text();
#ifdef XML_DEBUG
  qDebug() << name;
  ...
  case delegate::ItemType::ModbusItem: {
    ...
    bool status = false;
    auto parent =
        static_cast<BciNumber>(nextChildElement.text().toUInt(&status));
    if (!status)
      qWarning() << name << className;
    ...
    if (domNode.isElement()) {
      ...
      if (!domElement.isNull()) {
        ...
        if (domElement.tagName() == "module") {
          if (!isCorrectModule(domElement.attribute("mtypem", ""),
                               domElement.attribute("mtypeb", ""),
                               settings->startupInfoBlock.MTypeM,
                               settings->startupInfoBlock.MTypeB)) {
            domNode = domNode.nextSibling();
            continue;
          }
        }
        ...
        if (domElement.tagName() == "version") {
          if (settings->startupInfoBlock.isOutdated(
                  StdFunc::StrToVer(domElement.text()))) {
            throw std::runtime_error("Устаревшая версия ВПО, обновите ВПО");
          }
        }

        if (domElement.tagName() == "multimap") {
          if (domElement.attribute("name").contains("warn",
                                                    Qt::CaseInsensitive))
            settings->highlightWarn =
                parseMap<QMultiMap<quint32, quint32>>(domElement);
          else if (domElement.attribute("name").contains("crit",
                                                         Qt::CaseInsensitive))
            settings->highlightCrit =
                parseMap<QMultiMap<quint32, quint32>>(domElement);
        }
        ...
        if (domElement.tagName() == "protocom") {
          if (settings->interfaceType == Board::USB ||
              settings->interfaceType == Board::Emulator) {
            Protocom interface;
            if (settings->ifaceSettings.settings.isNull())
              settings->ifaceSettings = interface.parseSettings(domElement);
            else if (settings->ifaceSettings.settings
                         .canConvert<InterfaceInfo<Proto::ProtocomGroup>>()) {
              auto oldSettings =
                  settings->ifaceSettings.settings
                      .value<InterfaceInfo<Proto::ProtocomGroup>>();
              auto newSettings =
                  interface.parseSettings(domElement)
    ...
    if (domNode.isElement()) {
      ...
      if (!domElement.isNull()) {
        if (domElement.tagName() == keys::record) {
          ...
          BciNumber id =
              static_cast<BciNumber>(XmlParser::parseInt32(domElement));
...
void XmlParser::traverseNodeS2(const QDomNode &node,
                               QList<DataTypes::RecordPair> &settings,
                               config::widgetMap *widgets) {
  QDomNode domNode = node.firstChild();
  while (!domNode.isNull()) {
    if (domNode.isElement()) {
      QDomElement domElement = domNode.toElement();
...
void XmlParser::traverseNodeCheck(const QDomNode &node,
                                  std::vector<CheckItem> &settings) {
  QDomNode domNode = node.firstChild();
  while (!domNode.isNull()) {
    if (domNode.isElement()) {
      QDomElement domElement = domNode.toElement();
    ...
    if (domNode.isElement()) {
      ...
      if (!domElement.isNull()) {
        if (domElement.tagName() == "signals") {
          checkItem.signlsVec.emplace_back(
              XmlParserHelper::parseSignals(domElement));
          domNode = domNode.nextSibling();
          continue;
        }
...
template <typename T>
std::vector<T> XmlParser::parseVector(QDomElement domElement) {
  const auto &nodes = domElement.childNodes();
  std::vector<T> vector;
  Q_ASSERT(!nodes.isEmpty());
  int i = 0;
...
Container XmlParser::parseMap(QDomElement domElement) {
  Container map;
