...
namespace delegate {
enum class ItemType : int { ModbusItem };

struct Widget {
  Widget(const ctti::unnamed_type_id_t type_) : type(type_) {}
  Widget(const ctti::unnamed_type_id_t type_,
         const delegate::WidgetGroup group_)
      : type(type_), group(group_) {}
  Widget(const ctti::unnamed_type_id_t type_, const QString &desc_,
         const delegate::WidgetGroup group_, const QString &toolTip_)
...
struct DoubleSpinBoxWidget : Widget {
  DoubleSpinBoxWidget(const ctti::unnamed_type_id_t type_,
                      const delegate::WidgetGroup group_)
      : Widget(type_, group_) {}
  double min;
  double max;
...
struct DoubleSpinBoxGroup : DoubleSpinBoxWidget, Group {
  DoubleSpinBoxGroup(const ctti::unnamed_type_id_t type_,
                     const delegate::WidgetGroup group_)
...
struct CheckBoxGroup : Widget, Group {
  CheckBoxGroup(const ctti::unnamed_type_id_t type_,
                const delegate::WidgetGroup group_)
      : Widget(type_, group_) {}
...
struct QComboBox : Widget {
  enum PrimaryField : int { index = 0, data = 1, bitfield = 2 };
  QComboBox(const ctti::unnamed_type_id_t type_,
            const delegate::WidgetGroup group_)
      : Widget(type_, group_) {}
...
struct QComboBoxGroup : QComboBox, Group {
  QComboBoxGroup(const ctti::unnamed_type_id_t type_,
                 const delegate::WidgetGroup group_)
      : QComboBox(type_, group_) {}
...
struct Item : delegate::Widget {
  ...
  Item(const ctti::unnamed_type_id_t type_, const delegate::ItemType itype_)
      : Widget(type_), itemType(itype_) {}
  Item(const ctti::unnamed_type_id_t type_, const delegate::ItemType itype_,
       const BciNumber parent_)
      : Widget(type_), itemType(itype_), parent(parent_) {}
  Item(const ctti::unnamed_type_id_t type_, const delegate::ItemType itype_,
       const BciNumber parent_, const delegate::WidgetGroup group_)
