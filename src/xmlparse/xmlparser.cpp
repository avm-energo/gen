        ...
        if (domNode.isElement())
        {
            ...
            if (!domElement.isNull())
            {
                ...
                if (domElement.tagName() == "modbus")
                {
                    ...
                    {
                        settings->ifaceSettings = InterfaceParser::parseSettings<CommandsMBS::ModbusGroup>(domElement);
                    }
                ...
                if (domElement.tagName() == "protocom")
                {
                    if (settings->interfaceType == Board::USB || settings->interfaceType == Board::Emulator)
                    {
                        ...
                        if (settings->ifaceSettings.settings.isNull())
                            settings->ifaceSettings = InterfaceParser::parseSettings<Proto::ProtocomGroup>(domElement);
                        else if (settings->ifaceSettings.settings.canConvert<InterfaceInfo<Proto::ProtocomGroup>>())
                        {
                            ...
                            auto newSettings = InterfaceParser::parseSettings<Proto::ProtocomGroup>(domElement)
                ...
                if (domElement.tagName() == "iec60870")
                {
                    ...
                    {
                        //            IEC104 interface;
                        settings->ifaceSettings = InterfaceParser::parseSettings<Commands104::Iec104Group>(domElement);
                    }
