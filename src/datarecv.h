...
auto inline operator<<(std::array<std::array<T, N2>, N1> &array,
                       const QStringList &list) -> decltype(array) {
...
std::array<T, N> inline operator<<(std::array<T, N> &array,
...
template <typename T, size_t N>
std::array<T, N> inline operator<<(std::array<T, N> &array, const QString str) {
...
inline bool is_same(const S2DataTypes::DataRec &lhs,
                    const S2DataTypes::DataRec &rhs) {
  bool is_same_value = false;
  if ((lhs.header.id == rhs.header.id) &&
      (lhs.header.numByte == rhs.header.numByte))
...
public:
  ...
  template <typename T> struct true_type {
    static constexpr bool value =
        std_ext::is_variant_alternative<T, valueType>();
  };
  template <typename T, std::enable_if_t<true_type<T>::value, bool> = true>
  void insert(int key) {
...
public:
  ...
  DataRecV(const S2DataTypes::DataRec &record)
      : DataRecV(record, static_cast<const char *>(record.thedata)) {}
  DataRecV(const valueMap &_map, const S2DataTypes::DataRec &record,
           const char *rawdata);

  DataRecV(const S2DataTypes::DataRec &record, const char *rawdata)
      : DataRecV(map, record, rawdata) {}
  ...
  template <typename T,
            std::enable_if_t<valueMap::true_type<T>::value, bool> = true>
  ...
  size_t typeIndex() const { return data.index(); }
...
private:
  ...
  template <typename T>
  void helper(unsigned int numByte, const char *rawdata, valueType &data) {
    constexpr auto hash = ctti::unnamed_type_id<T>().hash();
    assert(sizeof(T) == numByte);
    data = *reinterpret_cast<const T *>(rawdata);
  }
  template <typename T,
            std::enable_if_t<std_ext::is_container<T>::value, bool> = true>
